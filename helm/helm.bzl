...
cat $(location @com_github_jmthvt_rules_helm//:runfiles_bash) >> $@
...
export HELM=\\$$(rlocation com_github_jmthvt_rules_helm/helm)
...
def helm_chart(name, srcs, update_deps = False, repositories = None, **kwargs):
    ...
    if repositories:
        for repo in repositories:
            counter += 1
            repo_adds.append("$(location @com_github_jmthvt_rules_helm//:helm) repo add bazel{} {}".format(counter, repo))
        repo_adds.append("$(location @com_github_jmthvt_rules_helm//:helm) repo update")
    if update_deps:
...
{repo_adds}
$(location @com_github_jmthvt_rules_helm//:helm) package {package_flags} $$CHARTLOC
mv *tgz $@
rm -rf .helm
""".format(
            repo_adds = "\n".join(repo_adds),
...
def _helm_cmd(cmd, args, name, helm_cmd_name, values_yaml = None, values = None):
    binary_data = ["@com_github_jmthvt_rules_helm//:helm"]
    if values_yaml:
...
def helm_release(name, release_name, chart, values_yaml = None, values = None, repository = None, version = None, namespace = ""):
    ...
    genrule_srcs = ["@com_github_jmthvt_rules_helm//:runfiles_bash"]
